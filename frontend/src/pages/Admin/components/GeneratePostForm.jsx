...
const GeneratePostForm = ({
    ...
    const handleGeneratePost = async (e) => {
        ...
        if (!title || !tone) {
            setError("Please fill al the required fields.")
            return;
        }
        ...
        try {
            // Call AI API to generate post
            const aiResponse = await axiosInstance.post(
                API_PATHS.AI.GENERATE_POST,
                {
                    title,
                    tone,
                }
...
//             } catch (error) {
//     console.error("Error generating post:", error); // 👈 see full error in console
...
//         setError("No response from server. Check your network or API.");
//     } else {
...
// }
  ...
  return <div className="w-[90vw] md:w-[35vw] p-7 flex flex-col justify-center">
    ...
    <form onSubmit={handleGeneratePost} className="flex flex-col gap-3">
        <Input 
