...
import HomeCommentInfoCard from './components/HomeCommentInfoCard'
...
const BlogPostView = () => {
  ...
  const handleAddReply = async () => {
    try {
      const response = await axiosInstance.post(
        API_PATHS.COMMENTS.ADD(postData._id),
        {
          content: replyText,
          parentComment: ""
        }
      ...
      setReplyText("")
      setShowReplyForm(false)
      fetchCommentsByPostId(postId._id)
    } catch (error) {
    ...
    <BlogLayout>
      {postData && (
        <>
          ...
          <meta property="og:image" content={postData.coverImageUrl} />
            ...
            <div className="col-span-12 md:col-span-8 relative">
              ...
              <div className="flex items-center gap-1 flex-wrap mt-3 mb-5">
                <span className="text-[13px] text-gray-500 font-medium">
                  {moment(postData.updatedAt || "").format("Do MMM YYYY")}
                </span>
              ...
              <img 
                ...
                className="w-full max-h-[500px] h-auto object-cover mb-6 rounded-lg"
              />

              <div>
                <MarkdownContent 
                  content={sanitizeMarkdown(postData?.content || "")}
                />

                <SharePost title={postData.title} />
                  ...
                  {comments?.length > 0 && 
                    comments.map((comment) => (
                      <HomeCommentInfoCard 
                        ...
                        getAllComments={() =>
                          fetchCommentsByPostId(postData._id)
                        }
                        onDelete={(commentId) =>
                        setOpenDeleteAlert({
                          open: true,
